(function (factory) {
    ...
    var Node = (function (_super) {
        ...
        Node.prototype.instrumentCode = function (code, filename, shouldCompile) {
            this.log('Instrumenting', filename);
            if (filename.endsWith('.d.ts')) {
                return code;
            }
            if (shouldCompile) {
                return util_2.transpileSource(filename, code);
            }
        ...
        Node.prototype._beforeRun = function () {
            ...
            return _super.prototype._beforeRun.call(this).then(function () {
                ...
                if ((config.environments.filter(isRemoteEnvironment).length > 0 &&
                    ...
                    return serverTask
                        .then(function () {
                        var tunnel = _this.tunnel;
                        _this._createSessionSuites();
                        return tunnel
                            .start()
        ...
        Node.prototype._createTunnel = function () {
            var _this = this;
            ...
            var TunnelConstructor = this.getTunnel(config.tunnel);
            var tunnel = new TunnelConstructor(this.config.tunnelOptions);
            this.tunnel = tunnel;
        ...
        Node.prototype._resolveConfig = function () {
            ...
            return _super.prototype._resolveConfig.call(this).then(function () {
                ...
                config.environments.forEach(function (env) {
                    var browserName = env.browserName;
                    var newName = getNormalizedBrowserName(browserName);
                ...
                if (config.tunnelOptions.drivers) {
                    config.tunnelOptions.drivers = config.tunnelOptions.drivers.map(function (driver) {
                        var driverName;
                        ...
                        var newName = getNormalizedBrowserName(driverName);
                        if (typeof driver === 'string') {
                            return newName;
                        }
                        if ('name' in driver) {
                            return tslib_1.__assign({}, driver, { name: newName });
                        }
                        return driver;
                    });
                }
                ...
                if (!require.extensions['.ts']) {
                    if (((config.node &&
                        config.node.suites.some(function (pattern) { return pattern.endsWith('.ts'); })) ||
                        (config.plugins &&
                            config.plugins.some(function (plugin) { return plugin.script.endsWith('.ts'); }))) &&
                        typeof _this.config.node.tsconfig === 'undefined') {
                        ts_node_1.register();
                    }
                    else if (_this.config.node.tsconfig) {
                        ts_node_1.register({ project: _this.config.node.tsconfig });
                    }
                }
                _this._instrumenter = istanbul_lib_instrument_1.createInstrumenter(Object.assign(tslib_1.__assign({ esModules: true, coverageVariable: config.coverageVariable }, config.instrumenterOptions), {
                ...
                var remoteEnvironments = config.environments.filter(isRemoteEnvironment);
                if (remoteEnvironments.length > 0 && config.tunnel && !config.serveOnly) {
                    var tunnel = _this._createTunnel();
                    ...
                    return tunnel.getEnvironments().then(function (tunnelEnvironments) {
                        var resolvedEnvironments = resolveEnvironments_1.default(config.capabilities, remoteEnvironments, tunnelEnvironments);
                        var localEnvironments = config.environments.filter(function (env) { return !isRemoteEnvironment(env); });
                        config.environments = localEnvironments.concat(resolvedEnvironments);
                    });
                }
            });
        };
        Node.prototype._getSeleniumDriverNames = function () {
            ...
            for (var _i = 0, _a = config.environments; _i < _a.length; _i++) {
                var env = _a[_i];
                var browserName = env.browserName;
                if (browserName === 'chrome' ||
                    ...
                    var browserVersion = env.browserVersion;
                    if ((!isNaN(browserVersion) && Number(browserVersion) < 1000) ||
            ...
            return new common_1.Task(function (resolve, reject) {
                ...
                uncoveredFiles.forEach(function (filename) {
                    try {
                        ...
                        _this.instrumentCode(code, filename, filename.endsWith('.ts') || filename.endsWith('.tsx'));
                    }
                    catch (_error) { }
                });
            });
    ...
    function getNormalizedBrowserName(name) {
        if (name === 'ie') {
